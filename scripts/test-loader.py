...
def main():
    ...
    with tempfile.TemporaryDirectory(dir=temp) as folder:
        ...
        for template in ['commands', 'panel_settings', 'menu_hooks', 'note_import', 'compile_report', 'tool_console']:
        ...
        for command in [[cmake, '-S', str(templates), '-B', str(templates / 'build'), '-G', generator, '-A', 'x64', f'-DHAMMER_ADDONS_SDK={root / "dist/sdk"}'],
            ...
            result = subprocess.run(command, capture_output=True, text=True, timeout=120)
            assert result.returncode == 0, result.stdout + result.stderr
        run(0, 'loaded=8 rejected=0 disabled=1', 'loaded generated_note_import', 'loaded generated_compile_report', 'loaded generated_tool_console')
