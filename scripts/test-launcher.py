...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix='launcher space & test-', dir=root / 'build/tests') as temporary:
        ...
        for name in ['Launch Workshop Tools.cmd', 'tools_launcher.exe', 'hammer_addons_runtime.dll',
                     'hammer_addons_ui.dll', 'README.md', 'LAUNCHER.md', 'BUILD_OUTPUT.md', 'TOOL_LOGS.md', 'addons/hello/addon.ini', 'addons/hello/hello.dll',
        ...
        assert all(f'loaded {name}' in log for name in ['hello', 'compile_report', 'tool_console']) and 'UI initialization queued' in log
