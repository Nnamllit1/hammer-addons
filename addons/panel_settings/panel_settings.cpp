...
static int HA_CALL interact(void*,const HA_InteractionV1* event,char* response,size_t capacity) {
    if (std::strcmp(event->phase,"panel.change")==0) {
        ...
        std::snprintf(response,capacity,"Hello %s. Settings survive restarting Workshop Tools.",*name ? name : "mapper");
    }
    return HA_HANDLED;
}
static int HA_CALL load(const HA_HostV1* api) {
    ...
    const HA_ControlV1 controls[]{
        {sizeof(HA_ControlV1),HA_LABEL,"intro","Example","Edit a field to save it; click Greet to use it.",nullptr},
        {sizeof(HA_ControlV1),HA_TEXT,"name","Name",name,nullptr},
        ...
        {sizeof(HA_ControlV1),HA_BUTTON,"greet","Greet",nullptr,nullptr}
    };
    const HA_ContributionV1 panel{sizeof(HA_ContributionV1),HA_PANEL,"preferences","all",
        "Example: panel and settings","","",controls,5,interact,nullptr};
    return ext->register_contribution(host->context,&panel)!=0;
}
extern "C" HA_EXPORT const HA_AddonV1* HA_CALL HA_Query(uint32_t abi) {
    static const HA_AddonV1 addon{sizeof(HA_AddonV1),HA_ABI_VERSION,"panel_settings",
